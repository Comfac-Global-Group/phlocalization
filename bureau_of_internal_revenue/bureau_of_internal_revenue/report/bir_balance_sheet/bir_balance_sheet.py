...
def filter_account_levels(data, selected_level):
...
def execute(filters=None):
	...
	LABEL_RENAMES = {
	"Total Asset (Debit)": "Total Current Assets (Debit)",
	"Total Liability (Credit)": "Total Current Liabilities (Credit)",
	}

	for row in data:
		label = row.get("account_name") or row.get("label") or row.get("name")
		new_label = LABEL_RENAMES.get(label)

		if not new_label:
			continue

		if "account_name" in row:
			row["account_name"] = new_label
		elif "label" in row:
			row["label"] = new_label
		elif "name" in row:
			row["name"] = new_label
	...
	if filters.get("level"):
		data = filter_account_levels(data, filters.get("level"))
