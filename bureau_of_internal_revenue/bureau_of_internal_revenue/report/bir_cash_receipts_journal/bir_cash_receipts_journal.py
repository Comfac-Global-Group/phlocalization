...
def get_columns():
	return [
		...
		{"label": _("Check Amount"), "fieldname": "check_amount", "fieldtype": "Currency", "width": 120},
		{"label": _("Other Charges"), "fieldname": "other_charges", "fieldtype": "Currency", "width": 120},
		{"label": _("CWT"), "fieldname": "cwt", "fieldtype": "Currency", "width": 100},
		{"label": _("Overpayment"), "fieldname": "overpayment", "fieldtype": "Currency", "width": 120},
		{"label": _("Accounts Receivable"), "fieldname": "accounts_receivable", "fieldtype": "Currency", "width": 130},
	]
...
def get_data(filters):
	query = """
		SELECT
			...
			0 AS other_charges,
			IFNULL((SELECT SUM(ped.amount) FROM `tabPayment Entry Deduction` ped WHERE ped.parent = pe.name AND ped.account LIKE '1604%%'), 0) AS cwt,
		...
		SELECT
			...
			IFNULL((SELECT SUM(jea4.debit_in_account_currency) FROM `tabJournal Entry Account` jea4 WHERE jea4.parent = je.name AND jea4.account LIKE '1203%%'), 0) AS cash_amount,
			IFNULL((SELECT SUM(jea4.debit_in_account_currency) FROM `tabJournal Entry Account` jea4 WHERE jea4.parent = je.name AND jea4.account LIKE '1203%%'), 0) AS check_amount,
			0 AS other_charges,
			IFNULL((SELECT SUM(jea5.debit_in_account_currency) FROM `tabJournal Entry Account` jea5 WHERE jea5.parent = je.name AND jea5.account LIKE '1604%%'), 0) AS cwt,
			0 AS overpayment,
			IFNULL((SELECT SUM(jea6.credit_in_account_currency) FROM `tabJournal Entry Account` jea6 WHERE jea6.parent = je.name AND jea6.account LIKE '1301%%'), 0) AS accounts_receivable
	...
	"""
	return frappe.db.sql(query, filters, as_dict=False)