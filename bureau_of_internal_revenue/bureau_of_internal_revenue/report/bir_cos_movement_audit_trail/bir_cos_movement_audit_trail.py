...
def get_data(filters):
	...
	WITH
		...
		accounts_in_scope AS (
			...
			WHERE a.account_type = 'Cost of Goods Sold'
		)
	...
	FROM (
		...
		FROM `tabGL Entry` g
			...
			LEFT JOIN (
				...
				ON per.parent = g.voucher_no
				AND g.voucher_type = 'Payment Entry'
			LEFT JOIN `tabJournal Entry` je
				ON je.name = g.voucher_no
				AND g.voucher_type = 'Journal Entry'
			LEFT JOIN `tabSales Invoice` si
				ON si.name = g.voucher_no
				AND g.voucher_type = 'Sales Invoice'
			LEFT JOIN `tabPurchase Invoice` pi
				ON pi.name = g.voucher_no
				AND g.voucher_type = 'Purchase Invoice'
			LEFT JOIN `tabPurchase Receipt` pr
				ON pr.name = g.voucher_no
				AND g.voucher_type = 'Purchase Receipt'
			LEFT JOIN `tabStock Reconciliation` sr
				ON sr.name = g.voucher_no
				AND g.voucher_type = 'Stock Reconciliation'
		WHERE g.docstatus = 1
			AND IFNULL(g.is_cancelled, 0) = 0
			AND g.company = %(company)s
			AND g.posting_date BETWEEN %(from_date)s AND %(to_date)s
		...
		SELECT
			...
			CASE WHEN SUM(g.debit) - SUM(g.credit) > 0
				THEN SUM(g.debit) - SUM(g.credit) ELSE 0 END AS net_debit,
			CASE WHEN SUM(g.credit) - SUM(g.debit) > 0
				THEN SUM(g.credit) - SUM(g.debit) ELSE 0 END AS net_credit,
			NULL AS balance_debit,
			NULL AS balance_credit,
			CONCAT(
				g.account,
